"""Recovery watch (2026-09-04): 437 SKUs exist on the platform but the
seller API refuses them ("SKU does not exist"). Support escalated the
class to their development team and the examples we reported have already
come back, so the rest should follow - the user asked to be NOTIFIED when
they do, without having to keep checking.
...
def main():
    ...
    if bucket >= 100:
        subject = "All 437 unaddressable SKUs have recovered"
        body = (
            f"Every one of the {len(baseline)} SKUs reported to OnBuy on 3 September is "
        ...
        body = (
            ...
            f"3 September now answer on the seller API; {len(refused)} still return "
            "\"SKU does not exist\".\n\n"
